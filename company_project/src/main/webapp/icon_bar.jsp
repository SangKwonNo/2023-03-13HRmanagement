...
<div class="icon">
	<ul class="first-list">
		...
		<c:if test="${session.getAttribute('id') ne 'admin'}">
			...
			<div class="fourth-icon" role="img" title="업무 및 근태" onclick="location.href='${ctx}/employee_main_schedule.do'"></div>
			<div class="fifth-icon" role="img" title="급여관리" onclick="location.href='${ctx}/employee_main_salary.do'"></div>
			<div class="last-icon" role="img" title="로그아웃" onclick="logout()"></div>
		</c:if>
		<script>
			function logout() {
